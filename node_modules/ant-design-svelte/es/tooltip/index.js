...
function getOpst(parameters) {
    return tslib_1.__assign({ placement: 'top', trigger: 'hover' }, parameters);
}
function getPos(target, placement) {
    ...
    if (placement.indexOf('top') > -1) {
        ...
        if (placement.indexOf('Left') > -1) {
            pos = tslib_1.__assign({}, pos, { transform: "translate(0, -100%)" });
        }
        if (placement.indexOf('Right') > -1) {
            pos = tslib_1.__assign({}, pos, { transform: "translate(-100%, -100%)" });
        }
    }
        ...
        if (placement.indexOf('Left') > -1) {
            pos = tslib_1.__assign({}, pos, { transform: "" });
        }
        if (placement.indexOf('Right') > -1) {
            pos = tslib_1.__assign({}, pos, { transform: "translate(-100%, 0)" });
        }
    }
    if (placement.indexOf('Left') > -1) {
        pos = tslib_1.__assign({}, pos, { left: rect.left });
    }
    else if (placement.indexOf('Right') > -1) {
        pos = tslib_1.__assign({}, pos, { left: rect.left + rect.width });
    }
...
export default function tooltip(node, parameters) {
    var opts = getOpst(parameters);
    var tooltip = null;
    var overHander = function () {
